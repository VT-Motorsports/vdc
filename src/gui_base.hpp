...
inline void gui_base(vehicle &car, tire_data &td){
	...
	ImGui::InputTextWithHint("##run.dat", "Put cornering run.dat filepath here", tc_path, IM_ARRAYSIZE(tc_path));
	if(ImGui::Button("Load from file##tire")){load_dat_cornering(td, tc_path, 82.7);}
